...
class HttpCoreMiddleware extends CoreMiddleware
{
    ...
    protected function handleNotFound(ServerRequestInterface $request): ResponseInterface
    {
        $format = [
            'success' => false,
            'code' => EasyCode::NOT_FOUND,
            'message' => t('easycmf.not_found'),
        ];
        return $this->response()->withHeader('Server', 'EasyCMF')
    ...
    protected function handleMethodNotAllowed(
        ...
        $format = [
            'success' => false,
            'code' => EasyCode::METHOD_NOT_ALLOW,
            'message' => t('easycmf.allow_method', ['method' => implode(',', $methods)]),
        ];
        return $this->response()->withHeader('Server', 'EasyCMF')
            ->withAddedHeader('content-type', 'application/json; charset=utf-8')
            ->withStatus(405)
