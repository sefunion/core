 ...
 */
#[Command]
class InstallProjectCommand extends EasyCommand
{
     ...
     * 安装命令.
     */
    protected ?string $name = 'easy:install';
    ...
    {
        parent::configure();
        $this->setHelp('run "php bin/hyperf.php easy:install" install EasyCMF system');
        $this->setDescription('EasyCMF system install command');

        $this->addOption('option', '-o', InputOption::VALUE_OPTIONAL, 'input "-o reset" is re install EasyCMF');
    }
    ...
    {
        $this->line('-----------------------------------------------------------', 'comment');
        $this->line('Hello, welcome use EasyCMF system.', 'comment');
        ...
        if ($dbAnswer) {
            ...
            switch ($dbtype) {
                case 'mysql':
                    ...
                    $dbname = $this->ask('please input database name, default:', 'db_easycmf');
    ...
    protected function installLocalModule()
    {
        ...
        $easy = make(Easy::class);
        $modules = $easy->getModuleInfo();
        foreach ($modules as $name => $info) {
            $this->call('easy:migrate-run', ['name' => $name, '--force' => 'true']);
            if ($name === 'System') {
                $this->initUserData();
            }
            $this->call('easy:seeder-run', ['name' => $name, '--force' => 'true']);
    ...
    {
        $this->line(PHP_EOL . ' EasyCMF set others items...' . PHP_EOL, 'comment');
        $this->call('easy:update');
        $this->call('easy:jwt-gen', ['--jwtSecret' => 'JWT_SECRET']);
        $this->call('easy:jwt-gen', ['--jwtSecret' => 'JWT_API_SECRET']);

        if (! file_exists(BASE_PATH . '/config/autoload/easycmf.php')) {
            $this->call('vendor:publish', ['package' => 'sef/easy']);
        }
        ...
        if ($downloadFrontCode) {
            ...
            if (\shell_exec('which git')) {
                \system('git clone https://github.com/sefunion/easycmf-vue.git ./web/');
            } else {
...
default password: admin123', $this->getInfo(), Easy::getVersion()), 'comment');
    }
