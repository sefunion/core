...
use Easy\Crontab\Mutex\RedisServerMutex;
use Easy\Crontab\Mutex\RedisTaskMutex;
use Easy\Crontab\Mutex\ServerMutex;
use Easy\Crontab\Mutex\TaskMutex;
...
class EasyExecutor
{
     ...
     * @throws \Psr\Container\NotFoundExceptionInterface
     */
    public function execute(EasyCrontab $crontab, bool $run = false): ?bool
    {
        if ((! $crontab instanceof EasyCrontab || ! $crontab->getExecuteTime()) && ! $run) {
            return null;
        }
        $diff = 0;
    ...
    protected function runInSingleton(EasyCrontab $crontab, \Closure $runnable): \Closure
    {
    ...
    protected function runOnOneServer(EasyCrontab $crontab, \Closure $runnable): \Closure
    {
        return function () use ($crontab, $runnable) {
    ...
    protected function decorateRunnable(EasyCrontab $crontab, \Closure $runnable): \Closure
    {
    ...
    protected function logResult(EasyCrontab $crontab, bool $isSuccess, $result = '')
    {
        if ($this->logger) {
            if ($isSuccess) {
        ...
        $data = [
            ...
            'status' => $isSuccess ? EasyModel::ENABLE : EasyModel::DISABLE,
