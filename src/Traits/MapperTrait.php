...
trait MapperTrait
{
    ...
    {
        if (! config('easycmf.remote_api_enabled')) {
            throw new EasyException('系统未启用【远程通用列表查询】', 500);
        }
        /* @var $model EasyModel */
        $model = $this->getModel();
        $query = null;
        ...
        return method_exists($this, 'handleItems')
            ? (new EasyCollection($this->handleItems($query->get($params['select'] ?? ['*']), $params)))->toArray()
    ...
    public function getTreeList(
        ...
        $params['_easycmf_tree'] = true;
        $params['_easycmf_tree_pid'] = $parentField;
    ...
    {
        // 对树型数据强行加个排序
        if (isset($params['_easycmf_tree'])) {
            $query->orderBy($params['_easycmf_tree_pid']);
        }
     ...
     * @return mixed
     */
    public function first(array $condition, array $column = ['*']): ?EasyModel
    {
    ...
    public function getModel(): EasyModel
    {
    ...
    {
        return (new EasyCollection())->import($dto, $this->getModel(), $closure);
    }
     ...
     */
    public function dynamicRelations(EasyModel &$model, &$config): void
    {
